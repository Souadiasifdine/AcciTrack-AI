...
Users/subhajyotibhowmik/Downloads/
Accident_Detection_Blynk_GSM_MPU6050/
Accident_Detection_Blynk_GSM_MPU6050.ino
#define BLYNK_PRINT Serial
#define BLYNK_HEARTBEAT 30
#define TINY_GSM_MODEM_SIM800
#define SIM800L_IP5306_VERSION_20190610
#include <TinyGsmClient.h> // https://github.com/vshymanskyy/
TinyGSM
...
String mobile_number = "+918101074980"; //Enter your Phone Number
...
// set GSM PIN, if any
#define GSM_PIN ""
const char apn[] = "live.vodafone.com"; //Change with your
network provider APN
const char user[] = "";
const char pass[] = "";
//Change this with your Blynk Credentials
#define BLYNK_TEMPLATE_ID "TMPL3IoWvbWdH"
#define BLYNK_TEMPLATE_NAME "Accident Location"
#define BLYNK_AUTH_TOKEN "fJz8Qcv1zKrEH_PusU7tI7-_xANGhkM0"
...
void setup()
{
 ...
 SerialMon.println(String("IP5306 KeepOn ") + (isOk ? "OK" :
"FAIL"));
...
void loop()
{
 ...
 if (digitalRead (buttonpin) == HIGH) //Triggered with Push
Button
 {
 Serial.println("I am not Safe");
 message_with_data = message2 + "Latitude = " +
String(latitude, 6) + "Longitude = " + String(longitude, 6) + "
Link: https://maps.google.com/maps?&z=15&mrt=yp&t=k&q=" +
...
orientation changes by between 80-100 degrees
...
trigger3count++;
if (trigger3count >= 10) {
 angleChange = pow(pow(gx, 2) + pow(gy, 2) + pow(gz, 2),
0.5);
 //delay(10);
 Serial.println(angleChange);
...
accident = false;
 ...
 message_with_data = message + "Latitude = " +
String(latitude, 6) + "Longitude = " + String(longitude, 6) + "
Link: https://maps.google.com/maps?&z=15&mrt=yp&t=k&q=" +
String(latitude, 6) + "," + String(longitude, 6);
 modem.sendSMS(mobile_number, message_with_data);
...
void displayInfo()
{
 ...
 Blynk.virtualWrite(V6, "https://maps.google.com/maps?
&z=15&mrt=yp&t=k&q=" + String(latitude, 6) + "," +
String(longitude, 6));
 Blynk.virtualWrite(V9, String(longitude, 6), String(latitude,
6));
 }
}
BLYNK_WRITE(V7) // this command is listening when something is
written to V7
{
 int pinValue = param.asInt(); // assigning incoming value from
pin V7 to a variable

 if (pinValue == 1)
 {
 Serial.println("Sent Location");
 message_with_data = message1 + "Latitude = " + String(latitude,
6) + "Longitude = " + String(longitude, 6) + " Link: https://
maps.google.com/maps?&z=15&mrt=yp&t=k&q=" + String(latitude, 6) +
"," + String(longitude, 6);
 modem.sendSMS(mobile_number, message_with_data);
 message_with_data = "";
 }
